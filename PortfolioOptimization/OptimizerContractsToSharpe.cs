...
public class OptimizerContractsToSharpe
{
    ...
    public decimal CalculateSharpeForOnePermutation(int[] array)
    {
        decimal sharpe = -1;

        decimal[] totalDailyPNLs = new decimal[_initialDataHolder.InitialData.Count];

        int j = 0;
        foreach (var row in _initialDataHolder.InitialData)
        {
            totalDailyPNLs[j] = 0;
            for (var i = 0; i < array.Length; i++)
            {
                totalDailyPNLs[j] += _initialDataHolder.InitialData[j].DailyAccumulatedPnlByStrategy[i] * array[i];
            }
